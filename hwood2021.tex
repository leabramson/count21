...
change if this tract is excluded.% from all analyses.
...
other CVRTM choices (Table \ref{tbl:weights}); none significantly affect our findings.
...
homelessness in Greater Hollywood has declined to close to 2019 levels.\footnote{917, 491 
...
\end{tabular}
%}
\caption*{Raw counts from each tract coded as in Table \ref{tbl:tractStats}. 
		Fractions reflect averages over multiple counters. $^{\ast}$Assessed 16 March.}
\label{tbl:allCount}
\end{table*}
...
\caption*{Estimates reflect PDF medians with 90\% CIs shown in parentheses. $^{\ast}$Assessed 16 March. }
\label{tbl:allPop}
\end{table*}
