...
2021, finding the number of people experiencing unsheltered homelessness to have fallen in those communities
...
28\% of census tracts. This phenomenon might drive subjective impressions that the 
state of homelessness has worsened. Given COVID-related contractions in health, hygiene, and social 
services, however, such perceptions may well be accurate despite a decline in raw numbers. 
Data from the Coordinated Entry System data will reveal if homelessness has dropped 
...
\noindent {\bf Results and checks:} Table \ref{tbl:summary}'s population estimates  %and Figure \ref{fig:wtComp} 
come from the count data with tallies for cars, vans, RVs, tents, and makeshift dwellings weighted by 
their average occupancy. Our baseline result uses the same %quoted in Table \ref{tbl:summary} and shown in teal in Figure \ref{fig:wtComp}
...
\noindent {\bf Comments:} The drop we find mainly reflects $\sim$30\% fewer lone adults seen on the 
